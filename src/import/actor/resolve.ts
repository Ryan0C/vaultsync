...
export function findActorByExternalId(externalId: string): any | null {
  const actors = game.actors?.contents ?? [];
  return actors.find((a: any) => a?.getFlag?.(MODULE_ID, FLAG_KEY) === externalId) ?? null;
}
