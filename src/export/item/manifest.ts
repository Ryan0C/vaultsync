/**
 * export/item/manifest.ts
 *
 * Maintains a world-scoped export manifest for Items.
 *
 * Goals:
 ...
 *
 * Write strategy:
 *  1) Attempt to write/overwrite:  exports/items/index.json
 *  2) If overwrite fails, write a versioned manifest under:
 *        exports/items/_manifest/index.<ts>.<rand>.json
 *
 * Read strategy:
 *  1) Try reading exports/items/index.json
 *  2) Else, find newest file under exports/items/_manifest/
 */
...
export interface ItemManifestItem {
  ...
  name?: string;
  type?: string; // weapon, spell, equipment, feat, etc.
  updatedAt?: string;
...
export interface ItemManifest {
  ...
  items: Record<string, ItemManifestItem>;
}
...
export interface ItemManifestUpdate {
  doc: any; // Item
  latestFile: string;
}

export async function updateItemManifest(
  updates: ItemManifestUpdate[],
  source: "data" = "data"
): Promise<ItemManifest> {
  if (!updates.length) return await readItemManifest(source);

  await ensureVaultSyncDirs(source);
  const manifest = await readItemManifest(source);
  ...
  for (const u of updates) {
    ...
    manifest.items[item.key] = item;
  }

  await writeItemManifest(manifest, source);
  return manifest;
}

export async function readItemManifest(source: "data" = "data"): Promise<ItemManifest> {
  await ensureVaultSyncDirs(source);
  const paths = getVaultSyncPaths();

  const stablePath = `${paths.exports}/items/index.json`;
  const versionedDir = `${paths.exports}/items/_manifest`;

  // 1) Try stable index.json
  try {
    const stable = await readJsonSafe<ItemManifest>(stablePath);
  ...
  if (newest) {
    const v = await readJsonSafe<ItemManifest>(`${versionedDir}/${newest}`);
...
export async function writeItemManifest(
  ...
  const stablePath = `${itemsDir}/index.json`;
  const versionedDir = `${itemsDir}/_manifest`;
  ...
  try {
    ...
    console.warn(`[${MODULE_ID}] item manifest overwrite failed; falling back to versioned`, msg);
  }
...
function defaultManifest(): ItemManifest {
  return {
    moduleId: MODULE_ID,
...
function normalizeManifest(m: ItemManifest): ItemManifest {
  return {
...
function toItem(doc: any, latestFile: string): ItemManifestItem {
...
async function uploadJsonFile(
  ...
  return await FP().upload(source, dir, file, {
